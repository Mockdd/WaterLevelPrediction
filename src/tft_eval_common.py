...
def forecast_median_series(forecast_ts) -> pd.Series:
    ...
    if hasattr(forecast_ts, "quantile"):
        try:
            med = forecast_ts.quantile(0.5)
            return med.to_series() if hasattr(med, "to_series") else pd.Series(med.values().flatten(), index=med.time_index)
        except Exception:
            pass
    if hasattr(forecast_ts, "to_dataframe"):
        ...
        if "0.5" in pdf.columns.astype(str):
            return pdf["0.5"]
        return pdf.iloc[:, 0]
...
def forecasts_to_long_df(
    ...
    *,
    ...
    rows: list[dict] = []
    # Darts: multi-step forecast index often spans horizon; group by origin if possible
    if hasattr(forecast_ts, "to_dataframe"):
    ...
    # fallback: single column series, lead=1 only
    for t, v in s.items():
        t = pd.Timestamp(t)
        if t < eval_start or t > eval_end:
            continue
