...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--stride", type=int, default=None, help="rolling origin 간격(시간). 기본=H")
...
def rolling_forecasts_for_station(
    ...
    *,
    ...
    origins = g.loc[(g["datetime"] >= eval_start) & (g["datetime"] <= last_origin), "datetime"]
    ...
    for origin in origins:
        ...
        try:
            ...
            if future_cols:
                g_fut = g[
                    (g["datetime"] > origin)
                    & (g["datetime"] <= origin + pd.Timedelta(hours=horizon))
                ]
                if len(g_fut) < horizon:
                    continue
                _, _, fut_cut = df_to_timeseries(g_fut, target_col, past_cols, future_cols)
            pred = model.predict(
                n=horizon,
                series=series_cut,
